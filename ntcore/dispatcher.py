...
class Dispatcher(object):
    ...
    def __init__(self, storage, conn_notifier, verbose=False, stream_debug=False):
    ...
    def _serverThreadMain(self):
        ...
        try:
            while self.m_active:
                ...
                conn = NetworkConnection(connection_uid, stream, self.m_notifier,
        ...
        try:
            ...
            while self.m_active:
                ...
                with self.m_user_mutex:
                    ...
                    self.m_reconnect_cv.wait_for(lambda: not self.m_active or self.m_do_reconnect)
        finally:
            self.m_networkMode = NT_NET_MODE_NONE
    ...
    def _clientHandshake(self, conn, get_msg, send_msgs):
        ...
        while True:
            ...
            if verbose:
                logger.debug("client %s: received assign str=%s id=%s seq_num=%s val=%s",
                             self_id, msg.str, msg.id, msg.seq_num_uid, msg.value)
