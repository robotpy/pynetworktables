...
def test_SetEntryTypeValueAssignNew(storage_empty, dispatcher, entry_notifier, is_server):
    storage = storage_empty
    ...
    dispatcher._queueOutgoing.assert_has_calls([
        call(Message.entryAssign("foo", 0 if is_server else 0xffff, 1, value, 0), None, None),
    ])
...
def test_SetEntryTypeValueAssignTypeChange(storage_populate_one, dispatcher, entry_notifier, is_server):
    ...
    dispatcher._queueOutgoing.assert_has_calls([
        call(Message.entryAssign("foo", 0 if is_server else 0xffff, 2, value, 0), None, None),
    ])
    entry_notifier.notifyEntry.assert_has_calls([
...
def test_SetEntryTypeValueDifferentValue(storage_populated, dispatcher, entry_notifier, is_server):
...
def test_SetEntryValueAssignTypeChange(storage_populate_one, dispatcher, entry_notifier):
    storage = storage_populate_one
...
def test_SetEntryValueDifferentValue(storage_populated, is_server, dispatcher, entry_notifier):
    storage = storage_populated
...
def test_SetDefaultEntryExistsSameType(storage_populate_one, dispatcher, entry_notifier):
    storage = storage_populate_one
...
def test_SetDefaultEntryExistsDifferentType(storage_populate_one, dispatcher, entry_notifier):
    storage = storage_populate_one
...
def test_populated_SetDefaultEntryEmptyName(storage_populated, dispatcher, entry_notifier, is_server):
    storage = storage_populated
...
def test_populated_SetDefaultEntryEmptyValue(storage_populated, dispatcher, entry_notifier, is_server):
    storage = storage_populated
...
def test_SetEntryFlagsDifferentValue(storage_populated, dispatcher, entry_notifier, is_server):
    storage = storage_populated
...
def test_GetEntryFlagsExist(storage_populate_one, dispatcher, entry_notifier, is_server):
    storage = storage_populate_one
...
def test_deleteAllEntries(storage_populated, dispatcher, entry_notifier):
    ...
    entry_notifier.notifyEntry.assert_has_calls([
        call(ANY, ANY, ANY, NT_NOTIFY_DELETE | NT_NOTIFY_LOCAL)
    ]*4)
...
def test_LoadPersistentAssign(storage_empty, dispatcher, entry_notifier, is_server):
    ...
    dispatcher._queueOutgoing.assert_has_calls([
        call(Message.entryAssign("foo", 0 if is_server else 0xffff, 1, entry.value, NT_PERSISTENT), None, None),
    ])
...
def test_LoadPersistentUpdateFlags(storage_populated, dispatcher, entry_notifier, is_server):
    storage = storage_populated
...
def test_LoadPersistentUpdateValue(storage_populated, dispatcher, entry_notifier, is_server):
    storage = storage_populated
...
def test_LoadPersistentUpdateValueFlags(storage_populated, dispatcher, entry_notifier, is_server):
    storage = storage_populated
    ...
    entry_notifier.notifyEntry.assert_has_calls([
        call(1, "foo2", entry.value, NT_NOTIFY_FLAGS | NT_NOTIFY_UPDATE | NT_NOTIFY_LOCAL)
    ])
...
def test_loadPersistent(storage_empty, dispatcher, entry_notifier):
    ...
    entry_notifier.notifyEntry.assert_has_calls([
        call(ANY, ANY, ANY, NT_NOTIFY_NEW | NT_NOTIFY_LOCAL)
    ]*22)
...
def test_ProcessIncomingEntryAssign0(storage_empty, dispatcher, entry_notifier, is_server, conn):
    storage = storage_empty
    value = Value.makeDouble(1.0)
    ...
    storage.processIncoming(
            Message.entryAssign("foo", entry_id, 0, value, 0),
            conn)
...
def test_ProcessIncomingEntryAssign1(storage_populate_one, dispatcher, entry_notifier, is_server, conn):
    storage = storage_populate_one
    value = Value.makeDouble(1.0)
...
def test_ProcessIncomingEntryAssignIgnore(storage_populate_one, dispatcher, entry_notifier, is_server, conn):
    storage = storage_populate_one
    value = Value.makeDouble(1.0)
...
def test_ProcessIncomingEntryAssignWithFlags(storage_populate_one, dispatcher, entry_notifier, is_server, conn):
    storage = storage_populate_one
    value = Value.makeDouble(1.0)
...
def test_DeletedDeleteAllEntries(storage_populate_one, dispatcher, entry_notifier, is_server):
    storage = storage_populate_one
